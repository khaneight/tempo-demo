...
7. **Identity comes from the passkey session, never the body.** `requireUser` (`src/lib/auth.ts`) derives the address from the credential's public key; the optional `x-wallet` header is honoured only for wallets recorded in the signed linked-wallets cookie.
...
| Auth | `src/lib/auth.ts`, `src/app/api/auth/[[...route]]` | Passkey ceremony from `accounts/server` backed by Postgres (`src/lib/kv-postgres.ts`); admin password → HMAC cookie; linked-wallets cookie. |
...
| Wallet UI | `src/app/wallet/page.tsx`, `src/components/*-dialog.tsx`, `wallet-switcher.tsx`, `src/lib/use-wallets.ts` | Deposit/withdraw/send are dialogs whose whole lifecycle (incl. passkey signing) stays in the popup; `useOrder` polls an order while pending. Multi-wallet switching reorders the SDK store (no passkey prompt). |
