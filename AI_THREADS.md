...
## Session 4 — 2026-09-16 — Publish, deploy, UI, multi-wallet, prod audit · prompts P10–P18

### How the AI was steered
- **Publish with a clean slate** (P10): git history had been erased earlier at my request; the AI verified `.gitignore` (added editor/tool dirs and logs), staged, grepped the staged tree for the issuer key and local secrets before the first commit, and pushed a single commit to the empty GitHub repo.
- **Deployment as a checklist, not a hand-off** (P11): CI on GitHub Actions (typecheck/lint/tests against a Postgres service container/`next build`/Docker build), Vercel `buildCommand = pnpm db:migrate && pnpm build` so migrations run on every deploy (using Neon's unpooled URL), passkey `RP_ID`/`ORIGIN` defaulting to the deployment URL, and a numbered checklist for the parts only I could click. The first CI run failed on a Next-generated global type (`LayoutProps`) that only exists after a build — fixed by typing the layout props explicitly.
- **Debugging prod from logs, not guesses** (P12–P13): I had prefixed the Neon vars with `TEMPO_` — the AI made the DB URL lookup accept both names. The 500 on `/api/auth/register/options` looked like a `DATABASE_URL` zod error; the AI pulled the Vercel runtime logs and showed *every* user-created env var was failing validation (they had been saved blank in the Vercel UI) while the Neon-injected ones passed. Fix: blank env vars are treated as unset with a readable error, real values were uploaded from the local `.env` with freshly generated production secrets, redundant blanks removed, redeployed, and the routes probed to 200/401.
- **UI iterated in three rounds** (P14–P16): Inter + JetBrains Mono; identicons; a wallet header with balance; deposit/withdraw/send as dialogs whose whole lifecycle (including passkey signing and the outcome) stays in the popup; a unified activity timeline backed by an incremental per-wallet index of on-chain Transfer logs (the RPC caps `eth_getLogs` at ~100k blocks, discovered by probing); a send flow with own-wallet picker, live address validation, recipient identicon and a confirmation screen. A bug surfaced by a screenshot — an expired server session left the "Session expired" card with no button — was fixed in the same pass.
- **Multi-wallet from the SDK's actual model** (P15, P17): the AI read the `accounts` Provider/Store source to find that connected accounts persist with an `activeAccount` index and that `accountsChanged` fires only when the *set* of addresses changes — so switching became a local reorder of the store (no passkey prompt), and the server side got a signed linked-wallets cookie so a switched wallet is accepted without re-authenticating. wagmi's `connect()` refusing while connected was also caught by reading the source and handled by calling the provider's `wallet_connect` directly.
- **The prod audit answered with numbers** (P18): supply 65.000000 was reconciled unit by unit — local ledger 50 − 10 = 40, prod ledger 25, wallets + fee AMM summing to 65. The "incorrect drift" was correct arithmetic exposing a wrong setup: both environments were issuing against one token. Fix chosen with me: a dedicated production token, and — instead of deleting prod's test rows — the ledger became **scoped per token** (orders, activity index, admin sums and sweep all filter on the active token; processing an order of another token is refused). Local rows were backfilled to the old token; prod's old rows are hidden by design. Prod now reads drift 0 / outsiders 0 with all its users listed.
