...
- **Users** are passkeys. Tempo verifies WebAuthn/P256 signatures at the protocol level, so the passkey *is* the account (address = `keccak256(pubX‖pubY)[12:]`). The same passkey ceremony (`accounts/server`) also issues the app's session cookie, so "connect wallet" and "log in" are one gesture and the server always derives the caller's address from the credential, never from the request body. **Identities.** A person registers with a **username + passkey**; that passkey is the identity's first wallet ("Main"). While signed in they can **add wallets** — each a new passkey with its own label — and any of the identity's passkeys signs the identity in. The mapping lives server-side (`identities` ↔ `users`, one row per wallet/passkey), so wallets and labels follow the username across devices. The passkey ceremony's `name` is overloaded on purpose: with no session on the request it is the username of a *new* identity (taken/invalid → the ceremony is rejected and the credential dropped); with a session it is the label of a *new wallet* for that identity. **Authorization follows the identity**: a session established by any wallet may act, via the `x-wallet` header, for any wallet of the same identity and no other. **Switching never prompts** when the SDK already holds the account in this browser (a local reorder of its store; wagmi follows `accountsChanged`); a wallet never used on this device asks for one confirmation. The passkey is asked for at first use on a device and for every transaction it signs — nothing else.
